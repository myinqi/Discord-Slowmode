...
class TryaDcsWebChatTests(unittest.TestCase):
    def test_payload_extracts_reply_and_mention_ids(self):
        content, reply_to, mention_ids, client_message_id = parse_web_chat_payload({
            ...
            "client_message_id": "message_12345678",
        })
        self.assertEqual(content, "hello")
        self.assertEqual(reply_to, "123456789012345678")
        self.assertEqual(mention_ids, ["111", "222"])
        self.assertEqual(client_message_id, "message_12345678")

    def test_payload_rejects_non_numeric_reply(self):
        _, reply_to, _, client_message_id = parse_web_chat_payload({
            "content": "hi", "reply_to": "nope", "client_message_id": "bad id",
        })
        self.assertIsNone(reply_to)
        self.assertIsNone(client_message_id)
    ...
    def test_repeated_command_with_distinct_message_ids_is_not_deduped(self):
        store = {}
        self.assertTrue(register_web_chat_fingerprint(
            1, "!teach", client_message_id="command_0001", now=10.0, store=store,
        ))
        self.assertTrue(register_web_chat_fingerprint(
            1, "!teach", client_message_id="command_0002", now=10.1, store=store,
        ))
        self.assertFalse(register_web_chat_fingerprint(
            1, "!teach", client_message_id="command_0002", now=10.2, store=store,
        ))
